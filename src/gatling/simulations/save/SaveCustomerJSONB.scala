...
class SaveCustomerJSONB extends Simulation {
  ...
  val scn: ScenarioBuilder = scenario("Save Customer using JSONB")
    .feed(randomFeeder)
    .exec(http("Save Customer with Alias")
    .post("/customers/aliases")
    .header("shopId", "${randomInt}")
    .header("Content-Type", "application/json")
    .body(ElFileBody("saveJSONB.json"))
    .check(status.is(200)))
